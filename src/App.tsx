...
// //             return (
// //               <div className="flex flex-col md:flex-row gap-5 items-stretch h-[320px] sm:h-[450px] md:h-[550px]">
// //                 {/* Left/Top vertical thumbnails "side slides" strip */}
// //                 <div className="flex flex-row md:flex-col gap-3 overflow-x-auto md:overflow-y-auto shrink-0 md:w-28 order-2 md:order-1 select-none scrollbar-none py-1">
...
// //               }}
// //               className="sm:col-span-2 md:col-span-5 space-y-12"
// //             >
// //               <div className="space-y-6">
...
// //                 <div className="bg-white/80 backdrop-blur-xl p-6 sm:p-8 md:p-10 rounded-[2.5rem] border border-gold/25 space-y-6 md:space-y-8 shadow-xl relative overflow-hidden group w-full hover:shadow-2xl hover:border-gold/40 transition-all duration-500">
...
// //                     <div className="w-20 h-20 md:w-24 md:h-24 rounded-full overflow-hidden border-4 border-gold shadow-xl ring-4 ring-gold/10 transform group-hover:scale-105 transition-transform duration-500 shrink-0 relative bg-neutral-100">
// //                       <img 
// //                         src="/images/owner.jpg" 
// //                         alt="Kartikay Pandey" 
// //                         className="w-full h-full object-cover object-[center_20%]"
// //                         onError={(e) => {
// //                           // Fallback to Unsplash avatar in case owner.jpg is missing in the preview environment
// //                           e.currentTarget.src = "https://images.unsplash.com/photo-1633332755192-727a05c4013d?q=80&w=1000&auto=format&fit=crop";
// //                         }}
// //                         referrerPolicy="no-referrer"
// //                       />
...
// //                     target="_blank"
// //                     rel="noopener noreferrer"
// //                     className="flex flex-col sm:flex-row items-center gap-4 text-gray-600 bg-luxury-cream/80 p-4 sm:p-5 rounded-2xl border border-gold/20 shadow-sm relative z-10 hover:border-gold/50 hover:bg-white transition-all group/phone w-full"
// //                   >
...
// //                 visible: { opacity: 1, y: 0, transition: { duration: 0.6, ease: "easeOut" } }
// //               }}
// //               className="sm:col-span-2 md:col-span-3 space-y-10"
// //             >
...
//     const text = `*New Booking Inquiry for Tara Luxury Homes*%0A%0A*Name:* ${formData.name}%0A*Phone:* ${formData.phone}%0A*Residence:* ${formData.residence}%0A*Check-in Date:* ${formData.startDate}%0A*Check-out Date:* ${formData.endDate}%0A*Duration:* ${daysCount} Day${daysCount > 1 ? 's' : ''}%0A*Message:* ${formData.message}`;
//     const waUrl = `https://wa.me/91${whatsappPrimary}?text=${text}`;
    ...
    <div className="min-h-screen flex flex-col selection:bg-gold/30">
      ...
      <nav className={`fixed top-0 w-full z-50 transition-all duration-500 ${scrolled || view === 'details' ? 'bg-luxury-cream/95 backdrop-blur-xl border-b border-gold/15 shadow-md py-3' : 'bg-[#03221a]/85 backdrop-blur-md border-b border-gold/15 py-4'}`}>
        ...
        {view === 'home' && (
          ...
          >
            <div className="max-w-md mx-auto flex items-center justify-start sm:justify-center gap-2.5 px-4 py-3 overflow-x-auto scrollbar-none select-none">
              {['Home', 'Apartments', 'Rating', 'Booking'].map((item) => (
                <a 
                  key={item} 
                  href={`#${item.toLowerCase()}`}
                  className={`text-[10px] font-bold tracking-widest uppercase transition-all duration-300 py-2.5 px-5 rounded-full border text-center shrink-0 min-w-[90px] ${
                    scrolled 
                      ? 'text-luxury-black border-gold/30 hover:bg-gold/10 hover:border-gold active:bg-gold/20' 
                      : 'text-white border-gold/20 hover:bg-white/10 hover:border-white/30 active:bg-white/20'
                  }`}
          ...
          {(() => {
            ...
            return (
              <div className="flex flex-col md:flex-row gap-5 items-stretch h-[380px] sm:h-[450px] md:h-[550px]">
                {/* Left/Top vertical thumbnails "side slides" strip */}
                <div className="flex flex-row md:flex-col gap-3 overflow-x-auto md:overflow-y-auto shrink-0 md:w-28 order-2 md:order-1 select-none scrollbar-none py-1 px-4 md:px-0">
                  {allHotelImages.map((img, idx) => (
                    <button
                      key={idx}
                      onClick={() => setActiveImageIndex(idx)}
